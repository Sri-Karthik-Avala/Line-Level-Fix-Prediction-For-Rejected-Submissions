...
def make_batches(items, bs, shuffle, rng):
    ...
    if shuffle:
        ...
        for s in range(0, len(idx), mega):
            chunk = sorted(idx[s:s + mega], key=lambda i: items[i]["n"])
            for b in range(0, len(chunk), bs):
                out.append(chunk[b:b + bs])
        rng.shuffle(out)
        return out
    order = sorted(idx, key=lambda i: items[i]["n"])
...
def predict(model, items):
    ...
    for bidx in make_batches(items, 32, False, None):
        ...
        p = torch.sigmoid(model(tok, slot, feat, stmt, smask, lmask)).numpy()
...
def extra_threshold(probs, ys, labs):
    scores = []
    for p, y, lb in zip(probs, ys, labs):
        cand = [(float(p[k]), float(y[k])) for k in range(len(p)) if lb[k] > 0.5]
        cand.sort(key=lambda z: -z[0])
        for pv, yv in cand[1:MAX_PER_PROG]:
            scores.append((pv, yv))
    scores.sort(key=lambda z: -z[0])
    tp = 0.0
    best_t = 1.1
    for k, (pv, yv) in enumerate(scores, 1):
        tp += yv
        if tp / k >= EXTRA_PREC:
            best_t = pv
    return best_t
...
def main():
    ...
    oof = [a / SEEDS for a in oof_acc]
    test_prob = [a / (FOLDS * SEEDS) for a in test_acc]
    thr = extra_threshold(oof, [it["y"] for it in tr_items], [it["lab"] for it in tr_items])
    print("extra threshold", thr, flush=True)

    rows = []
    for i, row in enumerate(test.itertuples()):
        p = test_prob[i]
        lb = te_items[i]["lab"]
        cand = [(float(p[k]), k + 1) for k in range(len(p)) if lb[k] > 0.5]
        cand.sort(key=lambda z: -z[0])
        sel = [cand[0][1]] if cand else []
        for v, k in cand[1:MAX_PER_PROG]:
            if v >= thr:
                sel.append(k)
