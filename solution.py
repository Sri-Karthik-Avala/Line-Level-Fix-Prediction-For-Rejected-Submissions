...
def prog_pick(acc, lab, denom):
    v = np.asarray(acc, dtype=np.float64) / float(denom)
    keep = lab > 0.5
    if not keep.any():
        return -1, 0.0
    v = np.where(keep, v, -1e18)
    e = np.exp(v - v.max()) * keep
    tot = e.sum()
    if tot <= 0.0:
        return int(np.argmax(v)), 0.0
    e = e / tot
    order = np.argsort(-e)
    second = float(e[order[1]]) if len(order) > 1 and keep[order[1]] else 0.0
    return int(order[0]), float(e[order[0]]) - second


def coverage_cut(conf, correct):
    n = len(conf)
    if n == 0:
        return -1.0
    base = float(sum(correct)) / n
    if base <= 0.0:
        return -1.0
    ...
    for rank, i in enumerate(order, 1):
        ...
        prec = cum / rank
        cov = rank / float(n)
        if cov >= 0.35 and prec >= 1.45 * base:
            k_ag = rank
        if cov >= 0.50 and prec >= 1.25 * base:
            k_cons = rank
    k = k_ag if k_ag else k_cons
    if k <= 0 or k >= n:
        return -1.0
    return conf[order[k - 1]]
...
def main():
    ...
    oof_top, oof_conf, oof_ok = [], [], []
    for i in range(len(tr_items)):
        t, c = prog_pick(oof_acc[i], tr_items[i]["lab"], SEEDS)
        oof_top.append(t)
        oof_conf.append(c)
        oof_ok.append(1.0 if (t >= 0 and tr_items[i]["y"][t] > 0.5) else 0.0)
    print("oof top1", round(sum(oof_ok) / max(len(oof_ok), 1), 4), flush=True)

    cut = coverage_cut(oof_conf, oof_ok)
    print("confidence cut", round(cut, 5), flush=True)

    rows = []
    kept = 0
    for i, row in enumerate(test.itertuples()):
        t, c = prog_pick(test_acc[i], te_items[i]["lab"], FOLDS * SEEDS)
        if t >= 0 and c >= cut:
            sel = [t + 1]
            kept += 1
        else:
            sel = []
        rows.append({"id": row.id, "lines": " ".join(str(x) for x in sel)})
    print("kept", kept, "of", len(rows), flush=True)
